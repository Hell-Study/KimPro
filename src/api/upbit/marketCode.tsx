...
interface MarketCodeTableProps {
  marketCodes: MarketCode[];
}

const MarketCodeTable = memo(function MarketCodeTable({
  marketCodes,
...
function MarketCode() {
  ...
  const [marketCodes, setMarketCodes] = useState<MarketCode[] | null>(null);
  ...
  async function fetchMarketCodes() {
    ...
    const result: MarketCode[] = await response.json();
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <MarketCodeTable marketCodes={marketCodes || []} />
      )}
    </>
  );
}
